...
import { extractClassNamesFromCss } from 'src/parsers/css-parser'
...
/**
 * Generates Tailwind classes by processing CSS through Tailwind's PostCSS plugin (async version)
 * This captures all classes including those generated by plugins
 *
 * @param configPath - Absolute path to Tailwind config file
 * @returns Promise resolving to Set of all generated class names
 */
export async function generateTailwindBuildClasses(
  configPath: string,
): Promise<Set<string>> {
  try {
    // Dynamic imports for PostCSS and Tailwind
    const postcss = (await import('postcss')).default
    const tailwindcss = (await import('tailwindcss')).default

    // Minimal CSS input that triggers Tailwind to generate all utilities and components
    const inputCss = '@tailwind utilities;\n@tailwind components;'

    // Process the CSS through Tailwind's PostCSS plugin
    ...
    const { extractClassNamesFromCss } = await import('./css-parser')
    const classes = extractClassNamesFromCss(result.css)

    return classes
  } catch (error) {
    logger.warn(
      `Failed to generate Tailwind build classes from "${configPath}"`,
      error,
    )
    return new Set()
  }
...
/**
 * Synchronous version of generateTailwindBuildClasses
 * Uses require instead of dynamic imports for synchronous loading
 * This is needed for ESLint rules which must be synchronous
 *
 * @param configPath - Absolute path to Tailwind config file
 * @returns Set of all generated class names
 */
export function generateTailwindBuildClassesSync(
  configPath: string,
): Set<string> {
  try {
    // IMPORTANT: Tailwind's PostCSS plugin is async and cannot be run synchronously
    // Solution: Use execSync to run a separate Node process that generates the CSS

    // Create a temporary inline script that processes Tailwind and outputs CSS
    const script = `
      const postcss = require('postcss');
      const tailwindcss = require('tailwindcss');
      const inputCss = '@tailwind utilities;\\n@tailwind components;';

      postcss([tailwindcss('${configPath.replace(/'/g, "\\'")}')]).process(inputCss, { from: undefined })
    ...
    // Execute the script synchronously and capture output
    let css = execSync(`node -e "${script.replace(/"/g, '\\"')}"`, {
      encoding: 'utf-8',
      maxBuffer: 10 * 1024 * 1024, // 10MB buffer for large CSS output
      timeout: 30000, // 30 second timeout
      // Suppress stderr to avoid Tailwind debug output appearing in terminal
...
/**
 * Cache for build-generated classes to avoid regenerating on every call
 * Key: config file path + modification time
 * Value: Set of generated class names
 */
const buildClassesCache = new Map<string, Set<string>>()

/**
 * Gets the cache key for build classes based on config file path and modification time
 * @param configPath - Absolute path to config file
 * @returns Cache key string
 */
function getBuildCacheKey(configPath: string): string | null {
  try {
    const stats = fs.statSync(configPath)
    return `${configPath}:${stats.mtimeMs}`
  } catch {
    return null
  }
}
...
export async function getTailwindClasses(
  ...
  // Determine config path and options
  const configPath =
  ...
  // Conditionally generate classes from Tailwind build
  let buildClasses = new Set<string>()
  if (includePluginClasses) {
    // Check cache first
    const cacheKey = getBuildCacheKey(resolvedConfigPath)
    if (cacheKey && buildClassesCache.has(cacheKey)) {
      buildClasses = buildClassesCache.get(cacheKey)!
    } else {
      // Generate fresh build classes
      buildClasses = await generateTailwindBuildClasses(resolvedConfigPath)

      // Cache the result
      if (cacheKey) {
        buildClassesCache.set(cacheKey, buildClasses)
        // Clean up old cache entries (keep only last 5)
        if (buildClassesCache.size > 5) {
          const firstKey = buildClassesCache.keys().next().value
          if (firstKey) {
            buildClassesCache.delete(firstKey)
          }
        }
      }
    }
